  ...
  return (
    <ThreadListItemPrimitive.Root className="group flex items-center">
      <ThreadListItemPrimitive.Trigger
        onClick={onSelect}
        className="w-full text-left px-3 py-2 text-sm hover:bg-accent transition-colors rounded-md truncate data-[active]:bg-accent data-[active]:font-medium"
      >
        <div className="flex items-center gap-2">
          <span className="truncate flex-1">
            <ThreadListItemPrimitive.Title fallback="New conversation" />
          </span>
          {/* Check icon visible only on active thread (parent Root has data-active) */}
